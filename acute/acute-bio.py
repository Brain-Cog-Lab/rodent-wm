...
def main_work():
    ...
    para=np.load('./ampa-nmda/ampa-nmda.npy')
    indes=para[:,[7,10]]
    zero_rows = np.all(indes == 0, axis=1)
    inde=np.where(zero_rows)[0] 
    para=para[inde]
    ...
    for one in para:
        for two in range(4):
            ...
            plt.savefig("./plt/bio-ampa-nmda/%d-%dampa%f_nmda%f.jpg"%(alpha_ampa,alpha_nmda,ampa_scale,nmda_scale))
            plt.close()
            ...
            np.savez('./plt/bio-ampa-nmda/%d-%dampa%f_nmda%f.npz'%(alpha_ampa,alpha_nmda,ampa_scale,nmda_scale),**array_dict)
