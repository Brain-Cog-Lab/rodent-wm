...
def main_work():
    ...
    para=np.load('./i-gaba/i-gaba.npy')
    ...
    for three in range(2):
        for two in range(10):
            ...
            C_ext = 800
            C_selection = int(f * C_ext)
            C_selection_i = int((f +two*0.1)* C_ext)
            rate_selection = 25 * Hz
            


            stimuli4 = TimedArray(np.r_[np.zeros(int(40)), np.ones(32), np.zeros(400)], dt=25 * ms)
            input_e_4 = PoissonInput(P_E, 's_AMPA_ext', C_selection, rate_selection, 'stimuli4(t)')
            input_i_4=PoissonInput(P_I, 's_AMPA_ext', C_selection_i, rate_selection, 'stimuli4(t)')
            ...
            plt.savefig("./ii/no-bio/+ii%f-%d.jpg"%(C_selection_i,three))
            plt.close()
            ...
            np.savez('./ii/no-bio/+ii%f-%d.npz'%(C_selection_i,three),**array_dict)
